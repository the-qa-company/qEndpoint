...
public class QEPCoreValueFactory implements ValueFactory {
	...
	public Value fromComponent(QEPComponent component) {
		return switch (component.getNodeType()) {
		case IRI -> new QEPCoreIRI(component);
		case BLANK_NODE -> new QEPCoreBNode(component);
		case LITERAL -> new QEPCoreLiteral(component);
		};
	}
	...
	public Literal createLiteral(XMLGregorianCalendar xmlGregorianCalendar) {
		return createLiteral(xmlGregorianCalendar.toXMLFormat(),
				CALENDAR_DATATYPE.get(xmlGregorianCalendar.getXMLSchemaType()));
	}
	...
	/**
	 * convert a rdf4j {@link Value} into a core component
	 *
	 * @param value value
	 * @return component
	 */
	public QEPComponent asQEPComponent(Value value) {
		...
		return value == null ? null : core.createComponentByString(value.toString());
	}

	/**
	 * convert a rdf4j {@link Statement} into a core statement
	 *
	...
	public QEPStatement asQEPStatement(Statement statement) {
		...
		return new QEPStatement(statement.getSubject(), statement.getPredicate(), statement.getObject(),
				statement.getContext());
	}

	/**
	 * convert a core statement into a rdf4j {@link Statement}
	 *
	 * @param triple statement
	 * @return statement
	 */
	public QEPStatement fromQEPStatement(QEPComponentTriple triple) {
		return new QEPStatement((Resource) fromComponent(triple.getSubject()),
				(IRI) fromComponent(triple.getPredicate()), fromComponent(triple.getObject()));
	}
