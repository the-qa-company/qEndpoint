...
import org.eclipse.rdf4j.sail.lucene.MyNumericLiteral;
 ...
 */
public class AbstractValueFactoryHDT implements ValueFactory {

  private HDT hdt;
  /**
   * "universal" ID for bnode prefixes to prevent blank node clashes (unique per classloaded
  ...
  public IRI createIRI(String iri) {
    return new SimpleIRIHDT(hdt, iri);
  }
  ...
  /**
   * Calls {@link #createIntegerLiteral(Number, IRI)} with the supplied value and {@link
  ...
  /**
   * Calls {@link #createNumericLiteral(Number, IRI)} with the supplied value and datatype as
   * parameters.
   */
  ...
  /**
   * Calls {@link #createFPLiteral(Number, IRI)} with the supplied value and {@link XMLSchema#FLOAT}
   * as parameters.
   */
  @Override
  ...
  /**
   * Calls {@link #createFPLiteral(Number, IRI)} with the supplied value and {@link
  ...
  protected Literal createNumericLiteral(Number number, IRI datatype) {
    ...
    return new MyNumericLiteral(number, datatype);
  }

  /**
   * Calls {@link ValueFactory#createLiteral(String, IRI)} with the String-value of the supplied
   * calendar and the appropriate datatype as parameters.
   *
  ...
  public Literal createLiteral(XMLGregorianCalendar calendar) {
    return createLiteral(
        calendar.toXMLFormat(), XMLDatatypeUtil.qnameToURI(calendar.getXMLSchemaType()));
  }

  /**
   * Converts the supplied {@link Date} to a {@link XMLGregorianCalendar}, then calls {@link
