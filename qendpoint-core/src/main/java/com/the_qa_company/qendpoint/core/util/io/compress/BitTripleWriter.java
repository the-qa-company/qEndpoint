...
/**
 * Compact triple write, use {@link com.the_qa_company.qendpoint.core.util.BitUtil#log2(long)} to find the y and z sizes
 * use {@link BitTripleReader} to unpack.
 * use {@link #appendTriple(TripleID)} or {@link #appendTriple(long, long, long)} to add a new triple, the subject ids
 * should be incremental (1, 2, etc.). At the end call {@link #end()} to end the stream. {@link #close()} is only
 * closing the {@link OutputStream}.
 *
...
public class BitTripleWriter implements Closeable {
	...
	public void appendTriple(long s, long p, long o) throws IOException {
		...
		if (lastX != s) {
			if (lastX + 1 != s) {
				throw new IllegalArgumentException(format("Invalid subject order, last subject was %d, but new is (%d, %d, %d)", lastX, s, p, o));
			}
	...
	public void end() throws IOException {
		...
		// the stream is at the end, we can close it and write a CRC after the alignment
		writer.close();
		crcOut.writeCRC();
	}
