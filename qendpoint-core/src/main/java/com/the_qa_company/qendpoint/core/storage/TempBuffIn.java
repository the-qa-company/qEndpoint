...
/**
 * A {@code TempBuffIn} adds functionality to another input stream-namely, the
 * ability to buffer the input and to support the {@code mark} and {@code reset}
 * methods. When the {@code TempBuffIn} is created, an internal buffer array is
 ...
 * The {@code mark} operation remembers a point in the input stream and the
 * {@code reset} operation causes all the bytes read since the most recent
 * {@code mark} operation to be reread before new bytes are taken from the
 * contained input stream.
 *
 * @author Arthur van Hoff
 * @since 1.0
 */
public class TempBuffIn extends FilterInputStream {
	 ...
	 * Unsafe.compareAndSetReference instead of AtomicReferenceFieldUpdater (or
	 * VarHandles) to reduce dependencies and improve startup time.
	 */
	private static final Unsafe U = Unsafe.getUnsafe();

	private static final long BUF_OFFSET = U.objectFieldOffset(TempBuffIn.class, "buf");
	 ...
	 */
	/*
	 * We null this out with a CAS on close(), which is necessary since closes
	 * can be asynchronous. We use nullness of buf[] as primary indicator that
	 * this stream is closed. (The "in" field is also nulled out on close.)
	 */
	...
	/**
	 * The current position in the buffer. This is the index of the next byte to
	 * be read from the {@code buf} array.
	 * <p>
	 * This value is always in the range {@code 0} through {@code count}. If it
	 * is less than {@code count}, then {@code buf[pos]} is the next byte to be
	 * supplied as input; if it is equal to {@code count}, then the next
	 * {@code read} or {@code skip} operation will require more bytes to be read
	 ...
	 * method was called.
	 * <p>
	 * This value is always in the range {@code -1} through {@code pos}. If
	 * there is no marked position in the input stream, this field is
	 * {@code -1}. If there is a marked position in the input stream, then
	 * {@code buf[markpos]} is the first byte to be supplied as input after a
	 * {@code reset} operation. If {@code markpos} is not {@code -1}, then all
	 * bytes from positions {@code buf[markpos]} through {@code buf[pos-1]} must
	 * remain in the buffer array (though they may be moved to another place in
	 * the buffer array, with suitable adjustments to the values of
	 * {@code count}, {@code pos}, and {@code markpos}); they may not be
	 * discarded unless and until the difference between {@code pos} and
	 * {@code markpos} exceeds {@code marklimit}.
	 */
	protected int markpos = -1;

	/**
	 * The maximum read ahead allowed after a call to the {@code mark} method
	 * before subsequent calls to the {@code reset} method fail. Whenever the
	 * difference between {@code pos} and {@code markpos} exceeds
	 * {@code marklimit}, then the mark may be dropped by setting
	...
	/**
	 * Creates a {@code TempBuffIn} and saves its argument, the input stream
	 * {@code in}, for later use. An internal buffer array is created and stored
	 * in {@code buf}.
	 *
	 * @param in the underlying input stream.
	 */
	public TempBuffIn(InputStream in) {
		this(in, DEFAULT_BUFFER_SIZE);
	}

	/**
	 * Creates a {@code TempBuffIn} with the specified buffer size, and saves
	 * its argument, the input stream {@code in}, for later use. An internal
	 * buffer array of length {@code size} is created and stored in {@code buf}.
	 *
	 * @param in   the underlying input stream.
	 * @param size the buffer size.
	 * @throws IllegalArgumentException if {@code size <= 0}.
	 */
	public TempBuffIn(InputStream in, int size) {
		super(in);
		if (size <= 0) {
	 ...
	 * locked method. This method also assumes that all data has already been
	...
	private void fill() throws IOException {
		...
		if (markpos == -1) {
			...
			if (markpos > 0) { /* can throw away early part of the buffer */
				...
				int nsz = ArraysSupport.newLength(pos, 1, /* minimum growth */
	...
	/**
	 * See the general contract of the {@code read} method of
	 * {@code InputStream}.
	 *
	...
	/**
	 * Read bytes into a portion of an array, reading from the underlying stream
	 * at most once if necessary.
	 */
	 ...
	 * {@link InputStream#read(byte[], int, int) read} method of the
	 * {@link InputStream} class. As an additional convenience, it attempts to
	 * read as many bytes as possible by repeatedly invoking the {@code read}
	 * method of the underlying stream. This iterated {@code read} continues
	 * until one of the following conditions becomes true:
	 * <ul>
	 * <li>The specified number of bytes have been read,
	 * <li>The {@code read} method of the underlying stream returns {@code -1},
	 ...
	 * indicate end-of-file then this method returns {@code -1}. Otherwise, this
	...
	/**
	 * See the general contract of the {@code skip} method of
	 * {@code InputStream}.
	 *
	 * @throws IOException if this input stream has been closed by invoking its
	 *                     {@link #close()} method, {@code in.skip(n)} throws an
	 *                     IOException, or an I/O error occurs.
	 */
	 ...
	 * in the buffer ({@code count - pos}) and the result of calling the
	...
	/**
	 * See the general contract of the {@code mark} method of
	 * {@code InputStream}.
	 *
	...
	/**
	 * See the general contract of the {@code reset} method of
	 * {@code InputStream}.
	 * <p>
	 * If {@code markpos} is {@code -1} (no mark has been set or the mark has
	 * been invalidated), an {@code IOException} is thrown. Otherwise,
	 * {@code pos} is set equal to {@code markpos}.
	 *
	 * @throws IOException if this stream has not been marked or, if the mark
	 *                     has been invalidated, or the stream has been closed
	 *                     by invoking its {@link #close()} method, or an I/O
	 *                     error occurs.
	 */
	 ...
	 * methods. The {@code markSupported} method of {@code TempBuffIn} returns
	 * {@code true}.
	 *
	...
	private long implTransferTo(OutputStream out) throws IOException {
		if (getClass() == TempBuffIn.class && markpos == -1) {
			int avail = count - pos;
			if (avail > 0) {
				if (isTrusted(out)) {
	 ...
	 * <li>does not leak a reference to the {@code byte[]} to non-trusted
	 * classes</li>
	 * <li>does not modify the contents of the {@code byte[]}</li>
	 * <li>{@code write()} method does not read the contents outside of the
	 * offset/length bounds</li>
	 * </ul>
	 *
	...
	private static boolean isTrusted(OutputStream os) {
		...
		return clazz == ByteArrayOutputStream.class || clazz == FileOutputStream.class
