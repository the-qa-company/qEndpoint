...
public class PipedCopyIteratorUnordered<T> extends PipedCopyIterator<T> {
	 ...
	 * @return the iterator
	 */
	public static <T> PipedCopyIterator<T> createUnorderedOfCallback(PipeCallBack<T> callbackRunner) {
	...
	public void addElement(T node) {
		ArrayBlockingQueue<QueueObject<T>> currentQueue = queue[threadLocalIndexW.get() % queue.length];
		try {

			if (currentQueue == focusQueue) {
				for (ArrayBlockingQueue<QueueObject<T>> queueObjects : queue) {
					if (queueObjects != focusQueue) {
						currentQueue = queueObjects;
						break;
					}
			...
			boolean success = currentQueue.offer(new ElementQueueObject(node));
			if (!success) {
				focusQueue = currentQueue;
				while (!success) {
					for (ArrayBlockingQueue<QueueObject<T>> queueObjects : queue) {
