...
/**
 * Thread safe namespace store backed up on disk, {@link #load()} operation load the namespace, {@link #save(boolean)}
			...
			try (InputStream is = new BufferedInputStream(Files.newInputStream(location))) {
				...
				if (reader == null) {
					...
					if (version < NS_VERSION) {
						throw new IOException(format("Unknown version: %s, current: %s",
								versionStr, currentVersionStr));
					} else {
						throw new IOException(format("This namespace data was created with an newest version: %s, current: %s",
								versionStr, currentVersionStr));
					}
				}
