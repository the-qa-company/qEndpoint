...
public class BinarySearch {
    ...
    public static long first(Dictionary dictionary, long low, long high, String string, TripleComponentRole role) {
        if (high >= low) {
            ...
            else if (c > 0) return first(dictionary, (mid + 1), high, string, role);
            else return first(dictionary, low, (mid - 1), string, role);
        }
        return -1;
    }
     ...
     * @return the index, or -1 for not found
     */
    public static long last(Dictionary dictionary, long low, long high, long n, String string, TripleComponentRole role) {
        if (high >= low) {
            long mid = (high + low) / 2;
            int c = -1;
            if (mid != n) {
            ...
            int c2 =
                    COMPARATOR.compare(
                            string,
                            dictionary.idToString(mid, role).toString().subSequence(0, 1));
            //            System.out.println("c"+c);
            //            System.out.println("c2 "+c2);
            if ((mid == n || c != 0) && c2 == 0) return mid;
            else if (c < 0) return last(dictionary, low, (mid - 1), n, string, role);
            else return last(dictionary, (mid + 1), high, n, string, role);
        }
        return -1;
    }
}
