...
public class LuceneSailBuilder {
    ...
    public Iterator<Statement> buildToRDF(Resource node) {
        ...
        if (reindexQuery != null) {
            list.add(vf.createStatement(node, SailCompilerSchema.LUCENE_TYPE_REINDEX_QUERY,
                    vf.createLiteral(reindexQuery)));
        }
