...
/**
 * Service optimizer that moves the SERVICE clause for wikibase:label to the right position
 */
            ...
            if (node instanceof Service) {
                ...
                if (serviceUrl instanceof IRI) {
                    if (serviceUrl.stringValue().equals("http://wikiba.se/ontology#label")) {
                        // optimize only if the service clause is in a triple pattern block
                        if (node.getParentNode() instanceof Join || node.getParentNode() instanceof LeftJoin) {
                            // find the latest join or optional in the block where the service clause appears
