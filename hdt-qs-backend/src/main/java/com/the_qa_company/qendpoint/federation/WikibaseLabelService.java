...
/**
 * This implements a federated service that mimics the Wikibase query service SERVICE clause <http://wikiba.se/ontology#label>,
 * this is achieved by overwriting the standard SERVICE clause for this particular url
 * check https://en.wikibooks.org/wiki/SPARQL/SERVICE_-_Label for a full description of the specification
 */
public class WikibaseLabelService implements FederatedService {
    ...
    @Override
    public CloseableIteration<BindingSet, QueryEvaluationException> select(Service service, Set<String> set, BindingSet bindingSet, String s) throws QueryEvaluationException {
        return null;
    }

    @Override
    public CloseableIteration<BindingSet, QueryEvaluationException> evaluate(Service service, CloseableIteration<BindingSet, QueryEvaluationException> closeableIteration, String s) throws QueryEvaluationException {
        TupleExpr tupleExpr = service.getArg();
        // currently implements only the automatic mode https://en.wikibooks.org/wiki/SPARQL/SERVICE_-_Label
        // the SERVICE clause contains only one triple pattern that specifies the language
        if (tupleExpr instanceof StatementPattern){
            ...
            if (statement.getPredicateVar().getValue().stringValue().equals("http://wikiba.se/ontology#language")){
                // the object must be a literal without language tag and without datatype
    ...
    private BindingSet expandBindingSet(BindingSet bindingSet, List<String> languages){
        ...
        // according to https://en.wikibooks.org/wiki/SPARQL/SERVICE_-_Label 3 properties have to be expanded
        IRI[] expantionProperties = new IRI[]{RDFS.LABEL, SKOS.ALT_LABEL, vf.createIRI("https://schema.org/description")};
        ...
        for (int e=0; e<3; e++){
            ...
            for (String name : bindingSet.getBindingNames()) {
                ...
                if (bindingSet.getBinding(name).getValue() instanceof Resource){
                    CloseableIteration<? extends Statement, QueryEvaluationException> iteration = tripleSource.getStatements((Resource) bindingSet.getBinding(name).getValue(), expantionProperties[e],null);
