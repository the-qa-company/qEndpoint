...
public class HDTConverter {
    ...
    public Value IdToObjectHDTResource(long objectID){
        if (objectID >= endpoint.getHdtProps().getStartLiteral() && objectID <= endpoint.getHdtProps().getEndLiteral()) {
    ...
    public Value objectHdtResourceToResource(Value object){
        ...
        if (newObj instanceof SimpleIRIHDT) {
            ...
            if (literal.getLanguage().isPresent()){
                newObj = this.endpoint.getChangingStore().getValueFactory().createLiteral(((SimpleLiteralHDT)newObj).getLabel(),((SimpleLiteralHDT)newObj).getLanguage().get());
            } else {
                newObj = this.endpoint.getChangingStore().getValueFactory().createLiteral(((SimpleLiteralHDT)newObj).getLabel(),((SimpleLiteralHDT)newObj).getDatatype());
            }
        }
        return newObj;
    }
