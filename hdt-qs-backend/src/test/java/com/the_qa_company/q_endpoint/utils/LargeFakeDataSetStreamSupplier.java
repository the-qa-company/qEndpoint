...
import com.the_qa_company.q_endpoint.utils.sail.MultiLanguageTypedLuceneIndexTest;
...
public class LargeFakeDataSetStreamSupplier {
	...
	private IRI createPredicate() {
		return valueFactory.createIRI(
				"http://w" + random.nextInt() + "i.test.org/#Obj" + random.nextInt()
		);
	}

	private IRI createType() {
		return valueFactory.createIRI(
				"http://wti.test.org/#Obj" + random.nextInt(maxFakeType)
		);
	}
	...
	private Value createValue() {
		...
		if (random.nextBoolean()) {
			return valueFactory.createLiteral(text, MultiLanguageTypedLuceneIndexTest.stringNameOfInt(random.nextInt(Integer.MAX_VALUE)));
		} else {
	...
	private class FakeStatementIterator implements Iterator<Statement> {
		...
		public boolean hasNext() {
			...
			next = valueFactory.createStatement(
					createSubject(),
					createPredicate(),
					createValue()
			);

			long estimation = FileTripleIterator.estimateTripleSize(
				new TripleString(
