...
public class FunctionTest extends SailTest {
	...
	@Test
	@Ignore("TODO: find how to use tuple functions") // TODO: find how to use tuple functions
	public void splitFunctionTest() {
		add(
				VF.createStatement(
						VF.createIRI(SailTest.NAMESPACE + "s"),
						VF.createIRI(SailTest.NAMESPACE + "p"),
						VF.createLiteral("I love my plant2")
				)
		);
		Set<String> set = Sets.newHashSet("I love my plant2".split(" "));
		try (RepositoryConnection connection = repository.getConnection()) {
			TupleQuery query = connection.prepareTupleQuery(SailTest.joinLines(
					SailTest.PREFIXES,
					"SELECT ?element {",
					" ?s ex:p ?o .",
					" ?element <" + Split.URI + "> (?o, ' ') .",
	...
	public void dateFunctionTest() {
		...
		try (RepositoryConnection connection = repository.getConnection()) {
			TupleQuery query = connection.prepareTupleQuery(SailTest.joinLines(
					...
					"SELECT ?date {",
					" ex:s ex:p ?o .",
					" BIND (<" + ParseDateFunction.URI + ">(?o, 'yyyy-MM') AS ?date)",
					"}"
			));
			try (TupleQueryResult result = query.evaluate()) {
