...
public class RDF4JClientTest {
	...
	public void testAcceptHeaderGuessedRight() {
		...
		try {
			try (RepositoryConnection clearCo = repo.getConnection()) {
				// clear all data before
				clearCo.remove((Resource) null, null, null);
			}
			...
			try (RepositoryConnection connection = repo.getConnection()) {
				try (TupleQueryResult resultSet = connection.prepareTupleQuery(query).evaluate()) {
					while (resultSet.hasNext()) {
						...
						Value val = b.getBinding("o").getValue();
						assertTrue(val + " isn't a literal! (" + val.getClass().getCanonicalName() + ")",
								val.isLiteral());
					}
				}
			}
