...
public class MultiTypeFilteringSailTest extends SailTest {
	...
	protected Sail configStore(EndpointStore endpoint) {
		...
		LinkedSail<LuceneSail> luceneSail1 = new LuceneSailBuilder()
				...
				.withId(NAMESPACE + "lucene1").withEvaluationMode(TupleFunctionEvaluationMode.NATIVE).buildLinked();
		...
		LinkedSail<LuceneSail> luceneSail2 = new LuceneSailBuilder()
				...
				.withId(NAMESPACE + "lucene2").withEvaluationMode(TupleFunctionEvaluationMode.NATIVE).buildLinked();

		// config the multi type filtering sail
		return new MultiTypeFilteringSail(endpoint, iri("typeof"),
				new MultiTypeFilteringSail.TypedSail(luceneSail1, iri("type1")),
				new MultiTypeFilteringSail.TypedSail(luceneSail2, iri("type2")));
	}
	...
	public void noSplitIndexingTest() {
		...
		add(type1, type2, type3, type4, join, stmt1, stmt2, stmt3, stmt4);

		// test query lucene 1
		assertSelect(new LuceneSelectWhereBuilder("r", "text").withIndexId("ex:lucene1").buildWithSelectWhereClause(),
				new SelectResultRow().withValue("r", stmt1.getSubject()));
		// test query lucene 2
		assertSelect(new LuceneSelectWhereBuilder("r", "text").withIndexId("ex:lucene2").buildWithSelectWhereClause(),
		...
		assertSelect(
				joinLines("SELECT * {", " {",
						new LuceneSelectWhereBuilder("r1", "text").withIndexId("ex:lucene1").build(), " } UNION {",
						new LuceneSelectWhereBuilder("r2", "text").withIndexId("ex:lucene2").build(), " }", "}"),
		...
		assertSelect(
				joinLines("SELECT * {", new LuceneSelectWhereBuilder("r1", "text").withIndexId("ex:lucene1").build(),
						new LuceneSelectWhereBuilder("r2", "text").withIndexId("ex:lucene2").build(),
						"  ?r1 ex:join ?r2 .", "}"),
				new SelectResultRow().withValue("r1", stmt1.getSubject()).withValue("r2", stmt2.getSubject()));
	}
	...
	public void splitIndexingTest() {
		...
		add(type1, type2, type3, type4);
		add(join);
		add(stmt1, stmt2, stmt3, stmt4);
