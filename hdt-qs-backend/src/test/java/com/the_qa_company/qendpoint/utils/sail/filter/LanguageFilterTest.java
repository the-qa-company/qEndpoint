...
public class LanguageFilterTest extends SailTest {
	...
	protected Sail configStore(EndpointStore endpoint) {
		LinkedSail<LuceneSail> luceneSail = new LuceneSailBuilder()
				...
				.withId(NAMESPACE + "fr_lucene").withEvaluationMode(TupleFunctionEvaluationMode.NATIVE).buildLinked();
		// basic implementation
		filter = new LanguageSailFilter("fr", false, true);
		return new FilteringSail(luceneSail, endpoint, filter);
	}
	...
	public void languageInjectionTest() {
		...
		assertSelect(SPO_QUERY, new SelectResultRow().withSPO(lit1), new SelectResultRow().withSPO(lit2),
				new SelectResultRow().withSPO(lit3));

		assertSelect(
				new LuceneSelectWhereBuilder("subj", "text").withIndexId("ex:fr_lucene").buildWithSelectWhereClause(),
